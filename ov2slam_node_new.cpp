...
int main(int argc, char** argv)
{
    ...
    if(argc < 2)
    {
       logger.log("Usage: rosrun ov2slam ov2slam_node parameters_files/params.yaml");
       return 1;
    }

    logger.log("Launching OV²SLAM...");
    ...
    logger.log("Loading parameters file : " + parameters_file + "...");

    const cv::FileStorage fsSettings(parameters_file.c_str(), cv::FileStorage::READ);
    if(!fsSettings.isOpened()) {
       logger.log("Failed to open settings file...");
       return 1;
    } else {
        logger.log("Parameters file loaded...");
    }
    ...
    logger.log("SLAM thread execution time = " + std::to_string(std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count()) + "[µs]");
    logger.log("SLAM thread execution time = " + std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count()) + "[ms]");
    logger.log("SLAM thread execution time = " + std::to_string(std::chrono::duration_cast<std::chrono::seconds>(end - begin).count()) + "[s]");
